...
interface UserProfile {
  ...
  preferredPaymentMethod?: string; // Add preferred payment method field
}
...
export default async function UserProfile({ params }: { params: { username: string } }) {
  // Get username from URL - properly await params to fix Next.js warning
  ...
  // Get the preferred payment method if it exists
  const preferredMethod = userData.profile.preferredPaymentMethod;
  ...
  const formattedPaymentOptions = userData.profile.paymentMethods
    ...
    .map(method => {
      ...
      // Check if this is the preferred method
      const isPreferred = preferredMethod === method.type;
      ...
      <div className="px-6 py-8 max-w-md mx-auto">
        {preferredMethod && (
          <div className="mb-4 text-center">
            <p className="text-sm text-[rgba(var(--accent-color),1)] font-medium">Preferred: {paymentMethodProps[preferredMethod]?.name}</p>
          </div>
        )}
